"""Run chapter 6 with API data or a local fixture.
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="拉取二期第六章接口数据并输出 Markdown/HTML/PDF")
    ...
    parser.add_argument("--output", default=None, help="输出目录；默认 Reports/chapter6_api_{job_id}_{calmonth}")
...
def print_response_summary(response: Dict[str, Any], raw_path: Path) -> None:
    has_error, message = check_chapter_response(
        response,
        module=6,
        expected_chapter_keywords=("费用分析",),
...
async def run(args: argparse.Namespace) -> None:
    ...
    output_dir = (
        Path(args.output)
        if args.output
        else BASE_DIR / "Reports" / f"chapter6_api_{args.job_id}_{args.calmonth}"
    )
    raw_path = output_dir / "raw" / f"employee_{args.job_id}_month_{args.calmonth}_module_6.json"
    ...
    if "error" in response and not args.fixture:
        raise SystemExit(f"第六章接口请求失败，且未提供 fixture: {response.get('message', '')}")
    ...
    final_md_path = output_dir / "chapter6_final_report.md"
    final_html_path = output_dir / "chapter6_final_report.html"
    final_pdf_path = output_dir / "chapter6_final_report.pdf"
    final_data_path = output_dir / "chapter6_final_data.json"
    checklist_path = output_dir / "chapter6_apipost_checklist.md"
    ...
    checklist_path.write_text(build_chapter6_apipost_checklist(stats), encoding="utf-8")
