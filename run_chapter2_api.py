"""Run chapter 2 with real API data.

Example:
    python run_chapter2_api.py --job-id 86002542 --calmonth 202606 --api-key xxx
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="拉取二期第二章接口数据并复用现有 HTML/PDF 格式输出")
    ...
    parser.add_argument("--output", default=None, help="输出目录；默认 Reports/chapter2_api_{job_id}_{calmonth}")
    ...
    parser.add_argument("--note", default="", help="客户版 Markdown/PDF 末尾固定说明")
    parser.add_argument(
        "--raw-input",
        default=None,
        help="使用已归档的真实 MOUDLE=2 响应；用于无 API key 环境的可重现生成",
    )
    return parser
...
async def run(args: argparse.Namespace) -> None:
    output_dir = (
        Path(args.output)
        if args.output
        else BASE_DIR / "Reports" / f"chapter2_api_{args.job_id}_{args.calmonth}"
    )
    raw_path = output_dir / "raw" / f"employee_{args.job_id}_month_{args.calmonth}_module_2.json"
    ...
    has_error, message = check_chapter_response(
        response,
        module=2,
        expected_chapter_keywords=("利润概况",),
    ...
    try:
        cleaned = normalize_chapter2_data(response, period=args.calmonth)
        final_markdown = build_chapter2_markdown(cleaned)
        if args.note:
            final_markdown = final_markdown.rstrip() + f"\n\n{args.note}\n"
    except ChapterDataError as e:
        raise SystemExit(str(e)) from e

    final_md_path = output_dir / "chapter2_final_report.md"
    final_html_path = output_dir / "chapter2_final_report.html"
    final_pdf_path = output_dir / "chapter2_final_report.pdf"
    final_data_path = output_dir / "chapter2_final_data.json"
    checklist_path = output_dir / "chapter2_apipost_checklist.md"
    ...
    save_json(final_data_path, cleaned.to_dict())
    checklist_path.write_text(build_apipost_checklist(cleaned), encoding="utf-8")

    print(f"客户版 Markdown: {final_md_path.resolve()}")
    print(f"客户版 HTML: {final_html_path.resolve()}")
    print(f"客户版 PDF: {final_pdf_path.resolve()}")
    print(f"字段证据: {final_data_path.resolve()}")
