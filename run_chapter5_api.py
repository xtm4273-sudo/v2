"""Run chapter 5 with API data or a local fixture.

Example:
    python run_chapter5_api.py --job-id 86002542 --calmonth 202606 --api-key xxx
    python run_chapter5_api.py --job-id 86002542 --calmonth 202606 --skip-api --fixture Data/fixtures/chapter5_mock.json
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="拉取二期第五章接口数据并输出 Markdown/HTML/PDF")
    ...
    parser.add_argument("--output", default=None, help="输出目录；默认 Reports/chapter5_strict_{job_id}_{calmonth}")
...
def print_response_summary(response: Dict[str, Any], raw_path: Path) -> None:
    has_error, message = check_chapter_response(
        response,
        module=5,
        expected_chapter_keywords=("应收分析",),
...
def print_stats(stats: Dict[str, Any], final_data_path: Path) -> None:
    omit_info = stats.get("省略判断", {})
    print(
        "数据统计: "
        f"逾期客户 {stats.get('逾期客户数', 0)} 个，"
        f"次月到期客户 {stats.get('次月到期客户数', 0)} 个，"
        f"减值损失客户 {stats.get('减值损失客户数', 0)} 个，"
        f"跳账龄客户 {stats.get('跳账龄客户数', 0)} 个，"
        f"财务费用客户 {stats.get('财务费用客户数', 0)} 个"
    )
    print(
        "省略判断: "
        f"{'省略' if omit_info.get('是否省略第五章') else '展示'} - "
        f"{omit_info.get('原因', '—')}"
    )
...
async def run(args: argparse.Namespace) -> None:
    ...
    output_dir = (
        Path(args.output)
        if args.output
        else BASE_DIR / "Reports" / f"chapter5_strict_{args.job_id}_{args.calmonth}"
    )
    raw_path = output_dir / "raw" / f"employee_{args.job_id}_month_{args.calmonth}_module_5.json"
    ...
    if "error" in response and not args.fixture:
        raise SystemExit(f"第五章接口请求失败，且未提供 fixture: {response.get('message', '')}")

    generation_source = response
    if args.fixture:
        fixture_path = Path(args.fixture)
        generation_source = load_json(fixture_path)
        print(f"使用已保存的原始接口响应生成报告: {fixture_path.resolve()}")

    # raw 目录始终保存实际用于生成报告的完整响应，避免 --fixture 时留下空壳。
    save_json(raw_path, generation_source)
    print_response_summary(generation_source, raw_path)

    from ReportGenerator.chapter5_strict import build_chapter5_apipost_checklist, format_chapter5_strict
    from ReportGenerator.chapter5_renderer import save_final_html, save_final_pdf

    try:
        final_markdown, stats = format_chapter5_strict(generation_source, period=args.calmonth)
    except ChapterDataError as e:
        raise SystemExit(str(e)) from e

    final_md_path = output_dir / "chapter5_final_report.md"
    final_html_path = output_dir / "chapter5_final_report.html"
    final_pdf_path = output_dir / "chapter5_final_report.pdf"
    final_data_path = output_dir / "chapter5_final_data.json"
    checklist_path = output_dir / "chapter5_apipost_checklist.md"
    ...
    checklist_path.write_text(build_chapter5_apipost_checklist(stats), encoding="utf-8")
    print_stats(stats, final_data_path)

    if stats.get("省略判断", {}).get("是否省略第五章"):
        print("按规则省略第五章，不生成 HTML/PDF。")
