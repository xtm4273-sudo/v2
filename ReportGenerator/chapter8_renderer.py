...
def _markdown_to_pdf(markdown: str, output_path: Path) -> None:
    ...
    styles.add(ParagraphStyle(name="CNTitle8", parent=styles["Title"], fontName=font, fontSize=20, leading=28, alignment=TA_CENTER, textColor=colors.HexColor("#16324f"), spaceAfter=12))
    styles.add(ParagraphStyle(name="CNSection8", parent=styles["Heading2"], fontName=font, fontSize=14, leading=19, alignment=TA_LEFT, textColor=colors.HexColor("#244b73"), spaceBefore=10, spaceAfter=6))
    styles.add(ParagraphStyle(name="CNBody8", parent=styles["BodyText"], fontName=font, fontSize=10.5, leading=17, spaceAfter=5))
    ...
    while i < len(lines):
        ...
        if stripped.startswith("<!--"):
            ...
            continue
        if line.startswith("# "):
            story.append(Paragraph(_inline_pdf(line[2:]), styles["CNTitle8"]))
        elif line.startswith("## "):
            story.append(Paragraph(_inline_pdf(line[3:]), styles["CNSection8"]))
        elif line.startswith("### "):
            story.append(Paragraph(_inline_pdf(line[4:]), styles["CNSection8"]))
        else:
            story.append(Paragraph(_inline_pdf(line), styles["CNBody8"]))
        i += 1
    ...
    pdf = SimpleDocTemplate(str(output_path), pagesize=A4, rightMargin=18 * mm, leftMargin=18 * mm, topMargin=18 * mm, bottomMargin=18 * mm, title="第八章总结报告")
